...
extension MainPageViewController {
    func requestGetWeather() {
        WeatherService.shared.requestGetWeather(lat: +37.28043530, lon: +127.11463210, location: "용인시") { [weak self]  weather in
            if let weather = weather {
                self?.myLocationWeather.append(weather)
                self?.setViewControllerList()
                self?.setPageViewController(index: 0)
            }
        }
    }
}
